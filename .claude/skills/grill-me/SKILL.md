...
- Before starting a complex feature (especially one with high blast radius)
...
1. **File path** → read it
2. **Jira key (e.g., CG-123)** → fetch the issue via the Atlassian MCP
3. **Confluence page reference** → fetch via Atlassian MCP (CommonGround locked design lives in space CG)
...
- Reference real code — cite file paths and symbols when challenging an assumption
...
- **Edge cases** — empty states, concurrent users, offline, plugin missing/disabled
- **Architecture fit** — does this conflict with the hub-and-plugins boundary or the locked design in Confluence space CG?
- **Reversibility** — how hard is it to change this decision later?
- **Cross-platform** — does this hold on iOS, Android, web, desktop where Flutter targets diverge?
